...
unsigned int num_threads = 4; //This defines the max number of current threads possible
...
ThreadControlBlock* dequeue() {
	int i;
	ThreadControlBlock *tem;
	tem = mainThreadQueue->items[0];
	mainThreadQueue->items[0] = NULL;
	for (i = 0; i < mainThreadQueue->rear; i++) {
		mainThreadQueue->items[i] = mainThreadQueue->items[i + 1];
	}
	return tem;
}

void enqueue(ThreadControlBlock *x) {
	if (mainThreadQueue->rear == num_threads - 1) {
		printf("full");
	} else {
		mainThreadQueue->rear = (mainThreadQueue->rear + 1) % num_threads;
		mainThreadQueue->items[mainThreadQueue->rear] = x;
	}
}
*/
